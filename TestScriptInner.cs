  ...
  public static void CreateBindings(
    ...
    foreach (var node in scene.Nodes) {
      ...
      if (node.Type == "AnimationPlayer" && scene.AnimationLibrary != null && scene.AnimationLibrary.Animations != null) {
        ...
        newFileContent += $"      get {{\n";
        newFileContent += $"        if (_{instanceName} == null) {{\n";
        newFileContent += $"          _{instanceName} = parent.GetNode<{node.Type}>(\"{node.FullPath}\");\n";
        newFileContent += $"        }}\n";
        newFileContent += $"        return _{instanceName};\n";
        newFileContent += $"      }}\n";
        newFileContent += $"    }}\n";
        newFileContent += $"\n";
...
";
    ...
    newFileContent += $"    get {{\n";
    newFileContent += $"      if (_Nodes == null) {{\n";
    newFileContent += $"        _Nodes = new {className}Nodes(this);\n";
    newFileContent += $"      }}\n";
    newFileContent += $"      return _Nodes;\n";
    newFileContent += $"    }}\n";
    newFileContent += $"  }}\n";
    newFileContent += "}\n";
